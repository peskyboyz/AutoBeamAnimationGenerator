...
-- Dynamic DCT Gear Indicator with automatic range calculation and ignition level check
...
local function calculateGearRanges(numGears)
    -- Total positions = P, R, N, D, S + numGears (1, 2, 3, ... numGears)
    local totalPositions = 5 + numGears
    local rangeSize = 1.0 / totalPositions
    
    -- Calculate midpoint boundaries between positions
    gearRanges = {
        P = {min = 0, max = rangeSize * 0.5},
        R = {min = rangeSize * 0.5, max = rangeSize * 1.5},
        N = {min = rangeSize * 1.5, max = rangeSize * 2.5},
        D = {min = rangeSize * 2.5, max = rangeSize * 3.5},
        S = {min = rangeSize * 3.5, max = rangeSize * 4.5}
    }
end
...
local function updateGFX(dt)
    ...
    if ignitionLevel > 0 then
        ...
        -- Park
        if gearA >= gearRanges.P.min and gearA < gearRanges.P.max then
        ...
        -- Only check gearIndex if NOT in Drive or Sport mode
