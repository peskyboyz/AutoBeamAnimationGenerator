...
-- Custom Supercharger Boost Gauge with asymmetric scale support
...
-- Note: superchargerBoost is in Pascal, so we'll convert to PSI for easier configuration
...
local BOOST_MAX = 14.5    -- Maximum boost (typically 14.5 PSI or 1 bar, adjust for your supercharger)
...
-- Conversion constant: Pascal to PSI
local PASCAL_TO_PSI = 0.000145038

local currentBoost = 0
local smoothingFactor = 0.15

local function onInit()
    electrics.values['superchargerboost_asym'] = 0
    currentBoost = 0
end
...
local function updateGFX(dt)
    local superchargerBoost = electrics.values.superchargerBoost or 0  -- In Pascal
    local ignitionLevel = electrics.values.ignitionLevel or 0
    
    -- Convert from Pascal to PSI
    local boostPSI = superchargerBoost * PASCAL_TO_PSI
    
    -- Smooth the boost value for realistic gauge movement
    currentBoost = currentBoost + (boostPSI - currentBoost) * smoothingFactor
    ...
    electrics.values['superchargerboost_asym'] = normalizedBoost
    
    -- Optional: expose actual boost PSI for debugging
    electrics.values['superchargerboost_psi'] = currentBoost
end
